...
$(function() {
    ...
    $(window).bind('mousedown', function(event) {
        ...
        if ((content.clientWidth - event.clientX) < 40 && (content.clientWidth - event.clientX) >= 0) {
            //console.log(content.clientWidth-event.clientX);
			if(startY < $('.wsx_scroll')[0].offsetTop){
				var scrollTop = $('.wsx_scroll')[0].offsetTop - 50;
				var scroll_top = scrollTop / ($(window).height() - $('.wsx_scroll').height()) * ($(document).height() - $(window).height());
				$(document).scrollTop(scroll_top);
				return;
			}
			if(startY > ($('.wsx_scroll')[0].offsetTop + $('.wsx_scroll').height())){
				var scrollTop = $('.wsx_scroll')[0].offsetTop + 50;
				var scroll_top = scrollTop / ($(window).height() - $('.wsx_scroll').height()) * ($(document).height() - $(window).height());
				$(document).scrollTop(scroll_top);
				return;
        ...
        if ((content.clientHeight - event.clientY) < 40 && (content.clientHeight - event.clientY) >= 0) {
			if(startX < $('.niu_scroll')[0].offsetLeft){
				var scrollLeft = $('.niu_scroll')[0].offsetLeft - 50;
				var scroll_left = scrollLeft / ($(window).width() - $('.niu_scroll').width()) * ($(document).width() - $(window).width());
				$(document).scrollLeft(scroll_left);
				return;
			}
			if(startX > ($('.niu_scroll')[0].offsetLeft + $('.niu_scroll').width())){
				var scrollLeft = $('.niu_scroll')[0].offsetLeft + 50;
				var scroll_left = scrollLeft / ($(window).width() - $('.niu_scroll').width()) * ($(document).width() - $(window).width());
				$(document).scrollLeft(scroll_left);
				return;
